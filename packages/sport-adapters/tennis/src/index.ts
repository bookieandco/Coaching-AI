...
export function createTennisAdapter(options: TennisAdapterOptions = {}): SportAdapter {
  ...
  return {
    ...
    normalizeEvent(input: RawSportEvent): Event {
      ...
      return { eventId: (suppliedId ?? `tennis-event-${input.timestampMs ?? "unknown"}-${input.type}`) as Event["eventId"], eventType: input.type, actorParticipantId: typeof input.payload.winnerParticipantId === "string" ? input.payload.winnerParticipantId : undefined, timestampMs: input.timestampMs, payload: input.payload, evidenceRefs: [] };
    },
    normalizeAction(input: RawSportAction): Action {
      ...
      return { actionId: suppliedId ?? `tennis-action-${input.timestampMs ?? "unknown"}-${input.type}`, actorParticipantId: input.actorParticipantId, actionType: input.type, timestampMs: input.timestampMs, parameters: input.parameters ?? {}, evidenceRefs: [] };
    },
    applyEvent(state: GameState, event: Event): StateTransition {
      ...
      if (!winnerId || current.participants.length !== 2 || !current.participants.some((p) => p.participantId === winnerId)) return { fromVersion: current.stateVersion, toVersion: current.stateVersion, eventOrActionId: event.eventId, state: current, changedPaths: [] };
      ...
      const next = { ...result.state, stateVersion: current.stateVersion + 1, phase: { id: `set-${result.state.attributes.completedSets.length + 1}`, label: `Set ${result.state.attributes.completedSets.length + 1}`, sequence: result.state.attributes.completedSets.length + 1 } };
      return { fromVersion: current.stateVersion, toVersion: next.stateVersion, eventOrActionId: event.eventId, state: next, changedPaths: ["attributes.tennisScore", "attributes.currentSetTiebreak", "attributes.advantageParticipantId", "attributes.completedSets", "score", "phase", "evidenceRefs"] };
    },
    applyAction(state: GameState, action: Action): StateTransition {
      return this.applyEvent(state, { eventId: action.actionId as Event["eventId"], eventType: action.actionType, actorParticipantId: action.actorParticipantId, timestampMs: action.timestampMs, payload: action.parameters, evidenceRefs: action.evidenceRefs });
    },
    validateState(state: GameState): ValidationResult {
      ...
      for (const p of state.participants) { const s = scoreFor(match, p.participantId); if (s.games < 0 || s.games > 7 || s.sets < 0 || s.points < 0 || s.points > 40 || s.tiebreakPoints < 0) errors.push({ code: "INVALID_SCORE", message: `Invalid tennis score for ${p.participantId}.`, path: `attributes.tennisScore.${p.participantId}` }); }
    ...
    isTerminal(state: GameState): TerminalResult { const match = state as TennisMatchState; const target = Math.floor(match.attributes.bestOfSets / 2) + 1; const winner = match.participants.find((p) => scoreFor(match, p.participantId).sets >= target); return winner ? { terminal: true, reason: "match_complete", winnerParticipantId: winner.participantId } : { terminal: false }; },
  };
}
