...
export interface GamePhase { id: string; label: string; sequence: number; }
export interface GameState { gameId: GameId; sport: SportCode; rulesetVersion: string; participants: Participant[]; clock: GameClock; score: ScoreState; control: ControlState; phase: GamePhase; attributes: Record<string, unknown>; evidenceRefs: EvidenceRef[]; stateVersion: number; }
export interface Action { actionId: string; actorParticipantId?: string; actionType: string; timestampMs?: number; spatialContext?: unknown; parameters: Record<string, unknown>; evidenceRefs: EvidenceRef[]; }
export interface Event { eventId: EventId; eventType: string; actorParticipantId?: string; timestampMs?: number; payload: Record<string, unknown>; evidenceRefs: EvidenceRef[]; }
export interface StateTransition { fromVersion: number; toVersion: number; eventOrActionId: string; state: GameState; changedPaths: string[]; }
export interface ValidationResult { valid: boolean; errors: Array<{ code: string; message: string; path?: string }>; }
export interface SportMetadata { code: SportCode; name: string; rulesetVersion: string; }
export interface SportCapabilities { hasTeams: boolean; hasPossession: boolean; hasClock: boolean; hasSpatialModel: boolean; hasLineups: boolean; hasSubstitutions: boolean; hasPeriods: boolean; hasSets: boolean; hasRounds: boolean; hasInnings: boolean; hasContinuousPlay: boolean; supportsTacticalFormation: boolean; supportsPlayerTracking: boolean; supportsObjectTracking: boolean; }
export interface InitialStateInput { gameId: GameId; participants: Participant[]; rulesetVersion?: string; attributes?: Record<string, unknown>; }
export interface RawSportEvent { sourceType: string; type: string; timestampMs?: number; payload: Record<string, unknown>; }
export interface RawSportAction { type: string; actorParticipantId?: string; timestampMs?: number; parameters?: Record<string, unknown>; }
export interface ActionContext { state: GameState; objective?: string; }
export interface ActionDefinition { actionType: string; label: string; constraints: Record<string, unknown>; }
export interface ScoreContext { state: GameState; action: Action; }
