...
export interface CoachCommandBoundary {
  handle(query: CoachQuery): Promise<CoachResponse>;
}
...
export function validateCoachQuery(query: CoachQuery): string[] {
  ...
  if (query.kind === "explore_scenario" && !query.question?.trim() && !query.scenarioIds?.length) {
    errors.push("scenario exploration requires a question or scenarioIds");
  }
  if (query.kind === "compare_scenarios" && (query.scenarioIds?.length ?? 0) < 2) {
    errors.push("scenario comparison requires at least two scenarioIds");
  }
  return errors;
}
